...
export default function SettingsPage() {
  const { role } = useAuth();
  const { avatarKey, updateAvatar } = useAvatar();
  ...
  const handleAvatarChange = async () => {
    ...
    try {
      ...
      if (success) {
        toast({ title: "Avatar actualizado", description: "Tu avatar ha sido cambiado exitosamente." });
      } else {
        throw new Error('Failed to update avatar');
      }
    } catch {
      toast({ title: "Error", description: "No se pudo actualizar el avatar. Inténtalo de nuevo.", variant: "destructive" });
    }
  };
  ...
  const validatePasswordForm = () => {
    ...
    const v = validatePassword(passwordData.newPassword);
    if (!v.isValid) newErrors.newPassword = v.error!;
  ...
  const handleChangePassword = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        const err = await res.json();
        throw new Error(err.error || 'Error cambiando la contraseña');
      }
      ...
      <div className="flex flex-col gap-8 max-w-2xl mx-auto">
          ...
          <CardContent className="space-y-6">
            <div className="flex items-center gap-6">
               ...
               <div className="space-y-2">
                 ...
                 <p className="text-xs text-muted-foreground">Haga clic para generar un nuevo avatar aleatorio.</p>
               </div>
            </div>
            ...
            <div className="grid gap-2">
              ...
              <Input id="nickname" value={formData.nickname} onChange={(e) => handleInputChange('nickname', e.target.value)} placeholder="Tu apodo o nombre preferido" />
            </div>
            ...
            <div className="grid gap-2">
              ...
              <Input id="address" value={formData.address} onChange={(e) => handleInputChange('address', e.target.value)} placeholder="Tu dirección" />
            </div>

            <div className="grid gap-2">
              <Label htmlFor="email">Email</Label>
              <Input id="email" type="email" value={profileEmail} disabled className="bg-gray-50" />
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Cambia tu contraseña. Se recomienda usar una contraseña segura con al menos 6 caracteres.</CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            ...
            <div className="grid gap-2">
              ...
              <p className="text-xs text-muted-foreground">La contraseña debe tener entre 6 y 128 caracteres.</p>
            </div>
            <div className="grid gap-2">
          ...
          <CardContent className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <p className="text-sm text-muted-foreground">{role === 'admin' ? 'Administrador' : 'Empleado'}</p>
              </div>
              <div>
          ...
          <CardContent className="space-y-4">
            <div className="flex items-center justify-between">
              ...
              <Switch
                ...
                onCheckedChange={async (enabled) => {
                  ...
                  if (result.error) {
                    toast({ title: "Error", description: "No se pudo actualizar la configuración de notificaciones.", variant: "destructive" });
                  } else {
                    toast({ title: "Configuración actualizada", description: `Las notificaciones han sido ${enabled ? 'activadas' : 'desactivadas'}.` });
                  }
                }}
            ...
            <div className="flex items-center justify-between">
              <div className="space-y-0.5">
                ...
                <p className="text-sm text-muted-foreground">Elimina todas las notificaciones existentes.</p>
              </div>
              <Button
                variant="outline"
                size="sm"
                onClick={async () => {
                  ...
                  if (result.error) {
                    ...
                    toast({ title: "Notificaciones eliminadas", description: "Todas las notificaciones han sido eliminadas." });
                  }
                }}
              >
                Limpiar
